...
export default function ItemTable() {
  ...
  const filteredRows = questionCategories.filter((c) =>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Catégories de questions</h1>
              ...
              {filteredRows
                ...
                .map((questionCategory, index) => (
                  <tr key={index} className="transition-colors hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 font-medium text-foreground">{questionCategory.name}</td>
                    <td className="px-4 py-3 text-muted-foreground">{questionCategory.description}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => openEditModal(questionCategory)}
                          className="flex h-8 w-8 items-center justify-center rounded-lg border border-border text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                        >
                          <Pencil className="h-4 w-4" />
                        </button>
                        <button
                          onClick={() => deleteQuestionCategory(questionCategory._id)}
      ...
      <Modal isVisible={showCreateCategory} onClose={() => setShowCreateCategory(false)}>
        <QuestionCategory
          onSave={() => { displayQuestionCategories(); setShowCreateCategory(false); }}
      ...
      <Modal isVisible={showEditModal} onClose={closeEditModal}>
        <QuestionCategory
          ...
          onSave={() => { displayQuestionCategories(); closeEditModal(); }}
